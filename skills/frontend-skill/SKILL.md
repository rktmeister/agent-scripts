...
When working inside an existing product or design system, preserve the established visual language unless the user explicitly asks for a redesign.
...
- If the first screen includes a sticky or fixed header, that header counts against the hero. The combined header and hero content must fit within the initial viewport at common desktop and mobile sizes.
- When using `100vh` or `100svh` heroes, subtract persistent UI chrome (`calc(100svh - header-height)`) or overlay the header instead of stacking it in normal flow.
